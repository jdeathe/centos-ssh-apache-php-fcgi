...
describe "jdeathe/centos-ssh-apache-php-fcgi:latest"
	test_setup
	...
	describe "Basic Apache PHP operations"
		...
		it "Runs an Apache PHP container named apache-php.pool-1.1.1 on port ${DOCKER_PORT_MAP_TCP_80}."
			...
			it "Outputs Apache Details in the docker logs."
				...
				it "Includes the server name default (container name)."
				...
				it "Includes the server mpm default (worker)."
 ...
 - version_module"
			...
			it "Loads all the required Apache modules."
				...
				readonly other_required_apache_modules="core_module so_module http_module authz_host_module mpm_worker_module fcgid_module"
	...
	describe "Customised Apache PHP configuration"
		...
		it "Allows configuration of the application's package path."
			...
			docker exec \
				apache-php.pool-1.1.1 \
				mkdir -p -m 750 /opt/php-hw/{bin,etc/httpd/conf.d,etc/php.d,public_html,var/{log,tmp}}

			docker exec -i \
				apache-php.pool-1.1.1 \
				tee \
					/opt/php-hw/bin/php-wrapper \
					1> /dev/null \
					<<-EOT
			#!/usr/bin/env bash
			...
			docker exec \
				apache-php.pool-1.1.1 \
				find /opt/php-hw/bin -type f -exec chmod 750 {} +
