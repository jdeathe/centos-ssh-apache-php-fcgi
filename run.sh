...
if ! have_docker_container_name ${VOLUME_CONFIG_NAME} ; then
	...
	#	CMD=$(cp -R etc/services-config/ssh ${CONTAINER_MOUNT_PATH_CONFIG_SHARED_SSH}/)
	#	$CMD || sudo $CMD
	#fi
	...
	if [[ ! -n $(find ${CONTAINER_MOUNT_PATH_CONFIG}/httpd -maxdepth 1 -type f) ]]; then
		...
		# Disable mod_php if necessary
		if [[ -f ${CONTAINER_MOUNT_PATH_CONFIG}/httpd/conf.d/php.conf ]]; then
			CMD=$(mv ${CONTAINER_MOUNT_PATH_CONFIG}/httpd/conf.d/php.conf ${CONTAINER_MOUNT_PATH_CONFIG}/httpd/conf.d/php.conf.off)
			$CMD || sudo $CMD
		fi

		# The directory may exist but need to check the FCGID config is in place
		if [[ ! -f ${CONTAINER_MOUNT_PATH_CONFIG}/httpd/conf.d/fcgid.conf ]]; then
			CMD=$(cp etc/services-config/httpd/conf.d/fcgid.conf ${CONTAINER_MOUNT_PATH_CONFIG}/httpd/conf.d/)
			$CMD || sudo $CMD
		fi
	fi
